...
if (len(sys.argv) != 3):
	print 'ERROR!  Must have two arguments (mean, sigma) for halo weighing.'
	sys.exit()

deltaM = 0.1
mwMassMean = 10.**(np.log10(1.5*10.**11.) + (float(sys.argv[1])*deltaM))
#mwMassMean = float(sys.argv[1])*(10.**10.)
...
#File paths to save
saveFile = './halo_weighing_mInside.npy'
...
boolArray = (np.exp((-1.*((totalMassInside50 - mwMassMean)**2.))/(2.*(mwMassSigma**2.))))*(np.sqrt(2./np.pi))
